...
// Mappatura colonne Excel → chiavi payload
const EXCEL_COLS = [
...
// Converte "29/09/25" → "2025-09-29" (accetta anche "29/09/2025")
...
// Colori header per gruppo
const GRP_HEADER = [
  "#1e3a8a","#4c1d95","#14532d","#713f12","#7f1d1d","#14532d","#581c87"
];

function ReleaseScheduleSection({ contractId }) {
  const [records,     setRecords]     = useState([]);
  const [loadingRec,  setLoadingRec]  = useState(false);
  const [editing,     setEditing]     = useState(null); // null | "new" | rec
  ...
  const handleImport = async (e) => {
    ...
    try {
      ...
      // Legge come array di array (skipHeader: false → riga 0 = intestazioni)
      ...
      for (const cols of dataRows) {
        ...
        const rel = {
          ...
          devStart:   fmtCell(cols[1]),
          devEnd:     fmtCell(cols[2]),
          cfStart:    fmtCell(cols[3]),
          cfEnd:      fmtCell(cols[4]),
          e2eStart:   fmtCell(cols[5]),
          e2eEnd:     fmtCell(cols[6]),
          uatStart:   fmtCell(cols[7]),
          uatEnd:     fmtCell(cols[8]),
          certStart:  fmtCell(cols[9]),
          certEnd:    fmtCell(cols[10]),
          passInProd: fmtCell(cols[11]),
          dispClient: fmtCell(cols[12]),
        };
  ...
  // Colori alternati per celle dati per gruppo
  const cellBg = (gi, ri) => RELEASE_GROUPS[gi]?.color
    ? (ri % 2 === 0 ? RELEASE_GROUPS[gi].color + "55" : RELEASE_GROUPS[gi].color + "99")
    : (ri % 2 === 0 ? "#fff" : "#f8fafc");
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16, flexWrap: "wrap", gap: 10 }}>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
          <button onClick={openNew} disabled={!contractId}
            style={{ padding: "7px 14px", background: contractId ? "#0f172a" : "#94a3b8", color: "#fff", border: "none", borderRadius: 7, fontSize: 12, fontWeight: 700, cursor: contractId ? "pointer" : "not-allowed", letterSpacing: "0.2px" }}>
            + Nuova release
          </button>
          <label style={{ padding: "7px 14px", background: contractId && !importing ? "#0ea5e9" : "#94a3b8", color: "#fff", borderRadius: 7, fontSize: 12, fontWeight: 700, cursor: contractId ? "pointer" : "not-allowed", letterSpacing: "0.2px" }}>
          ...
          <button onClick={load} disabled={loadingRec}
            style={{ padding: "7px 10px", background: "#f1f5f9", border: "1px solid #cbd5e1", borderRadius: 7, fontSize: 13, cursor: "pointer", color: "#475569" }}
            title="Ricarica">{loadingRec ? "…" : "↻"}</button>
        </div>
      </div>
      ...
      <div style={{ overflowX: "auto", borderRadius: 8, border: "1px solid #e2e8f0" }}>
        <table style={{ borderCollapse: "collapse", fontSize: 12, whiteSpace: "nowrap", minWidth: "100%" }}>
          <thead>
            {/* Riga 1 — nomi gruppi */}
            <tr>
              <th rowSpan={2} style={{ padding: "10px 14px", textAlign: "left", verticalAlign: "middle", background: "#0f172a", color: "#fff", borderRight: "1px solid #1e3a5f", minWidth: 110, fontSize: 12, fontWeight: 800 }}>
                Release
              </th>
              {RELEASE_GROUPS.map((g, gi) => (
                <th key={g.group} colSpan={g.end ? 2 : 1} style={{
                  ...
                  background: GRP_HEADER[gi], color: "#fff",
              ...
              <th rowSpan={2} style={{ padding: "10px 10px", textAlign: "center", verticalAlign: "middle", background: "#0f172a", color: "#fff", fontSize: 11, minWidth: 80 }}>
                Azioni
              </th>
            </tr>
            {/* Riga 2 — Inizio / Fine */}
          ...
          <tbody>
            {records.length === 0 && !editing && !loadingRec && (
              <tr>
                <td colSpan={2 + RELEASE_GROUPS.reduce((s, g) => s + (g.end ? 2 : 1), 0)}
                  style={{ padding: "24px", textAlign: "center", color: "#94a3b8", fontSize: 13, background: "#fafafa" }}>
                  {contractId ? 'Nessuna release pianificata. Usa "+ Nuova release" o "↑ Importa Excel".' : "Seleziona un contratto."}
                </td>
              </tr>
            )}
            {records.map((rec, ri) => {
                ...
                return (
                  <tr key={rec.Id || rec.id} style={{ background: "#eff6ff", outline: "2px solid #3b82f6", outlineOffset: "-2px" }}>
                    ...
                    {RELEASE_GROUPS.flatMap(g => g.end ? [
                      <td key={g.start} style={{ padding: "3px 4px", borderRight: "1px solid #ddd6fe" }}>
                        <input type="date" style={{ ...inp, width: 118 }} value={draft[g.start] || ""} onChange={e => setDraft(d => ({ ...d, [g.start]: e.target.value }))} />
                      </td>,
                      <td key={g.end} style={{ padding: "3px 4px", borderRight: "1px solid #e2e8f0" }}>
                        <input type="date" style={{ ...inp, width: 118 }} value={draft[g.end] || ""} onChange={e => setDraft(d => ({ ...d, [g.end]: e.target.value }))} />
                      </td>,
                    ] : [
                      <td key={g.start} style={{ padding: "3px 4px", borderRight: "1px solid #e2e8f0" }}>
                        <input type="date" style={{ ...inp, width: 118 }} value={draft[g.start] || ""} onChange={e => setDraft(d => ({ ...d, [g.start]: e.target.value }))} />
                      </td>,
                    ])}
              ...
              return (
                <tr key={rec.Id || rec.id} style={{ borderBottom: "1px solid #e2e8f0" }}>
                  <td style={{ padding: "8px 14px", fontWeight: 700, color: "#0f172a", background: ri % 2 === 0 ? "#f8fafc" : "#fff", borderRight: "1px solid #e2e8f0", fontSize: 12 }}>
                    {rec.title}
                  </td>
                  {RELEASE_GROUPS.flatMap((g, gi) => g.end ? [
                    <td key={g.start} style={{ padding: "7px 8px", textAlign: "center", background: cellBg(gi, ri), borderRight: "1px solid rgba(0,0,0,0.06)", fontSize: 12, color: p[g.start] ? "#1e293b" : "#d1d5db", fontVariantNumeric: "tabular-nums" }}>
                      {formatDate(p[g.start])}
                    </td>,
                    <td key={g.end} style={{ padding: "7px 8px", textAlign: "center", background: cellBg(gi, ri), borderRight: "1px solid rgba(0,0,0,0.1)", fontSize: 12, color: p[g.end] ? "#1e293b" : "#d1d5db", fontVariantNumeric: "tabular-nums" }}>
                      {formatDate(p[g.end])}
                    </td>,
                  ] : [
                    <td key={g.start} style={{ padding: "7px 8px", textAlign: "center", background: cellBg(gi, ri), borderRight: "1px solid rgba(0,0,0,0.1)", fontSize: 12, color: p[g.start] ? "#1e293b" : "#d1d5db", fontVariantNumeric: "tabular-nums" }}>
                      {formatDate(p[g.start])}
                    </td>,
                  ])}
            ...
            {editing === "new" && (
              <tr style={{ background: "#f0fdf4", outline: "2px solid #22c55e", outlineOffset: "-2px" }}>
                ...
                {RELEASE_GROUPS.flatMap(g => g.end ? [
                  <td key={g.start} style={{ padding: "3px 4px", borderRight: "1px solid #dcfce7" }}>
                    <input type="date" style={{ ...inp, width: 118 }} value={draft[g.start] || ""} onChange={e => setDraft(d => ({ ...d, [g.start]: e.target.value }))} />
                  </td>,
                  <td key={g.end} style={{ padding: "3px 4px", borderRight: "1px solid #e2e8f0" }}>
                    <input type="date" style={{ ...inp, width: 118 }} value={draft[g.end] || ""} onChange={e => setDraft(d => ({ ...d, [g.end]: e.target.value }))} />
